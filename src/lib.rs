...
impl<T> Inner<T> {
    fn with<R>(&self, f: impl FnOnce(&mut Meta) -> R) -> R {
        #[cfg(feature = "parking_lot")]
...
impl<T> CstMutex<T> {
    ...
    pub fn shrink_to(&self, min_capacity: usize) {
        let garbage = self.inner.with(|meta| meta.pool.split_off(min_capacity));
        drop(garbage);
    }
    ...
    pub fn reserve(&self, additional: usize) {
        ...
        self.inner.with(|meta| meta.pool.merge(delta));
    }
}
